...
prx.types.metro_dropdown_withselection = {
    ...
    ,onDisplay: function(item,containerid,symbol) {
        ...
        if(prx.componentsHelper.getProp(item.withSelection,'boolean')) {
            ...
            cR += (typeof(item.buttons[item.selected]) != "undefined") ? prx.componentsHelper.getProp(item.buttons[item.selected].text,'text-textarea') : (typeof(item.buttons[0]) != "undefined") ? prx.componentsHelper.getProp(item.buttons[item.buttons[0]].text,'text-textarea') : '';
            cR += '</span>';
        }
        cR += '</div>'
